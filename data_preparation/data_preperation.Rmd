...
#### Total_waste column created

```{r}
DSNY <- DSNY %>% mutate(total_waste = select(.,4:6) %>% rowSums(na.rm = TRUE))
```
...
```{r}
DSNY_train <- DSNY %>% filter(month >= as.Date("2005-01-01") & month < as.Date("2021-01-01"))
DSNY_train_long <- DSNY %>% filter(month < as.Date("2021-01-01"))
```

```{r}
save(DSNY_train, DSNY_train_long,DSNY_third, file = "data_prep_boroughs.Rdata")
```
...
```{r}
#save(DSNY_train, DSNY_third, DSNY_third_bronx, file = "data_prep.Rdata")
```
...
It turns out this approach is no different from the DSNY_train data-frame. The totalwaste per communitydistrict, per month was already calculated. 
...
```{r}
bx_ts %>% ACF(total_waste, lag_max = 36) %>% autoplot()
```
...
### Report of ARIMA100 & ARIMA300 & stepwise model (ARIMA(0,0,3))
...
print("---")
bx_total_waste_fit %>% select(.model = stepwise) %>% report()
```
$$y_t = $$
...
```
## Attempt at a seasonal model

```{r}
bx_total_waste_fit_seasonal <- bx_ts %>% 
  model(arima003 = ARIMA(total_waste ~ pdq(0,0,3)), # This model has a constant and the Seasonal MA(1) component is not included 
        arima003s = ARIMA(total_waste ~ 1+ pdq(0,0,3) + PDQ(0,0,1)), # This model has a constant and the Seasonal MA(1) component is included 
        arima003s12 = ARIMA(total_waste ~ 1+ pdq(0,0,3) + PDQ(0,0,12)),
        arima003s12_no_mean = ARIMA(total_waste ~ pdq(0,0,3) + PDQ(0,0,12)), # This model has a constant and the Seasonal MA(12) component is included. 12 for the number of months
        stepwise = ARIMA(total_waste),
        search = ARIMA(total_waste, stepwise = FALSE, approximation = FALSE))
```

#### accuracy scores of this seasonal model

```{r}
accuracy(bx_total_waste_fit_seasonal)
```
```{r}
bx_total_waste_fit_seasonal %>% select(.model = search) %>% report()
print("---")
bx_total_waste_fit_seasonal %>% select(.model = arima003s) %>% report()
print("---")
bx_total_waste_fit_seasonal %>% select(.model = arima003s12) %>% report()
print("---")
bx_total_waste_fit_seasonal %>% select(.model = arima003s12_no_mean) %>% report()
```

My attempt at adding a seasonal component returned the same model as the ARIMA(0,0,3), in terms of coefficients and accuracy scores. Not sure why that is. 
...
```{r}
bx_diff1_fit %>% select(.model = arima200) %>% report()
print("---")
bx_diff1_fit %>% select(.model = stepwise) %>% report()
print("---")
bx_diff1_fit %>% select(.model = search) %>% report()
```
...
## Creating the Bronx Testing Set

Be aware that we have not excluded the months of January and February of 2022 in this line of code

```{r}
DSNY_test <- DSNY %>% filter(month >= as.Date("2021-01-01"))
# DSNY_train %>%
#   mutate(total_waste = select(.,4:6) %>% rowSums(na.rm = TRUE))
# DSNY %>% mutate(total_waste = select(., 4:6) %>%  rowSums(na.rm = TRUE))
```

## Goal: Run the fitted ARIMA models on the test set of BX values

### Let's filter out the Bronx values and 
...
bx_test = DSNY_test %>% 
  ...
  filter(borough_id == 2) %>% 
  summarise(., total_waste = sum(total_waste)) %>% 
  ungroup() # I added ungrouped at the end to avoid any potential errors in plots or manipulations later on
```
```{r}
bx_test <- DSNY_test %>% 
  group_by(month, borough) %>%
...
### Now lets convert it to a tsibble

```{r}
bx_test_ts <- as_tsibble(bx_test, index = 'month_num', regular = TRUE) %>% 
  tsibble::fill_gaps()

# bx_test %>% 
#   mutate(month_num = row_number()) %>% #seq.int(nrow(.)) also works as well
#   update_tsibble(index = month_num, regular = TRUE) %>% 
#   mutate(diff1 = difference(total_waste, differences = 1, order_by = month_num)) %>% 
#   tsibble::fill_gaps()
```

We will work with the index as 'month_num'

```{r}
bx_test_ts %>% 
  ggplot(mapping = aes(x = month, y = total_waste)) + geom_line() +
  labs(x = "Month & Year", y = "Total Waste", title = "Bronx Total Waste Collected")
```


### ACF plot for total_waste from, the test set

```{r}
bx_test_ts %>% ACF(total_waste) %>% autoplot()
```

I am attempting to create ARIMA models using the BX total waste values from the test set

```{r}
bx_test_total_waste_fit <- bx_test_ts %>% 
  model(arima003 = ARIMA(total_waste ~ 1 + pdq(0,0,3) + PDQ(0,0,0)), # This model has a constant and the Seasonal MA(1) component is not included 
        arima100 = ARIMA(total_waste ~ pdq(1,0,0)),
        arima110 = ARIMA(total_waste ~ pdq(1,1,0)),
        stepwise = ARIMA(total_waste),
        search = ARIMA(total_waste, stepwise = FALSE, approximation = FALSE))
```

Return the accuracy metrics for the models fitted onto the BX total waste test set

```{r}
bx_test_total_waste_fit %>% accuracy()
```

Return the coefficients of the models from above

```{r}
bx_test_total_waste_fit %>% select(.model = arima003) %>% report()
print("---")
bx_test_total_waste_fit %>% select(.model = arima100) %>% report()
print("---")
bx_test_total_waste_fit %>% select(.model = arima110) %>% report()
print("---")
bx_test_total_waste_fit %>% select(.model = stepwise) %>% report()
```


```{r}
bx_test_total_waste_fit %>%
  forecast(h = 8) %>%
  autoplot(bx_test_ts, level = NULL) + labs(x = "Month Number",
                                              y = "Total Waste")
```
The new model, using the parameters from the training models was created. Not enough testing data is used. We can see that the averaged total_waste values are beginning to stabilize right after the 16th month forecast. 
...
```{r}
unemp_rate <- read.csv("C:/Users/Daniel/Desktop/R/STAT_790/STAT_790_Case_Seminar/external_datasets/unemployment_rate_seasonally_adjusted.csv", header = T)
```
...
```{r}
cons_price_index <- read.csv("C:/Users/Daniel/Desktop/R/STAT_790/STAT_790_Case_Seminar/external_datasets/consumer_price_index_seasonally_adjusted.csv", header = T)
```
