...
bx_test_total_waste_fit <- bx_test_ts %>% 
  model(arima200 = ARIMA(total_waste ~ pdq(2,0,0)),
        arima100 = ARIMA(total_waste ~ pdq(1,0,0)),
...
print("---")
bx_test_total_waste_fit %>% select(.model = arima200) %>% report()
print("---")
bx_test_total_waste_fit %>% select(.model = arima110) %>% report()
print("---")
bx_test_total_waste_fit %>% select(.model = search) %>% report()
```
...
bx_test_total_waste_fit %>%
  forecast(h = 8) %>%
  autoplot(bx_test_ts, level = NULL)
```
