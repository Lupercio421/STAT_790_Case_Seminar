...
The STAT 715 project created the time series dataframe with the forecast package and the as.ts() function. I am attempting to approach this project with the fpp2 package and the as_tsibble() function. The gg_season plot from fpp2 is working for now on the bx_ts tsibble. However, the ggseasonplot from the forecast package will not work.
...
$r_1$ is often large and positive.
...
Here I created an initital ARIMA model on both the total_waste, and diff1 values for the BX. Still need to explore why for the total_waste values, I am not getting a recommendation of differencing the data. 
...
So the reason month_num is displayed because I updated the index of the tsibble to 'month_num' in chunch 41, which is indexed from 0-192. 
