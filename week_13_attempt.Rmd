...
final_nyc_data_small %>% 
  mutate(avg_precip_diff1 = difference(avg_precip, 
                                 differences = 1, 
                                 order_by = month_num)) %>% 
  ggplot(., 
         mapping = aes(x = month, 
                       y = avg_precip_diff1)) + geom_line() +
  geom_point(color="steelblue") + 
  labs(x = "Month",
       title = "Differenced Precipitation Values",
       subtitle = "Jan '05 - Dec '20")

# CPI
final_nyc_data_small %>% 
  mutate(cpi_diff1 = difference(cpi, 
                                 differences = 1, 
                                 order_by = month_num)) %>% 
  ggplot(., 
         mapping = aes(x = month, 
                       y = cpi_diff1)) + geom_line() +
  geom_point(color="steelblue") + 
  labs(x = "Month",
       title = "Differenced CPI Values",
       subtitle = "Jan '05 - Dec '20")

# Unemployment Rate
final_nyc_data_small %>% 
  mutate(unemp_diff1 = difference(unemp_rate, 
                                 differences = 1, 
                                 order_by = month_num)) %>% 
  ggplot(., 
         mapping = aes(x = month, 
                       y = unemp_diff1)) + geom_line() +
  geom_point(color="steelblue") + 
  labs(x = "Month",
       title = "Differenced CPI Values",
       subtitle = "Jan '05 - Dec '20")
...
final_nyc_data_small <- final_nyc_data_small %>% 
  mutate(tw_diff1 = difference(total_waste_total, 
         ...
         cdd_diff1 = difference(avg_CDD, 
                                 differences = 1, 
                                 order_by = month_num),
         avg_precip_diff1 = difference(avg_precip, 
                                 differences = 1, 
                                 order_by = month_num),
         cpi_diff1 = difference(cpi, 
                                 differences = 1, 
                                 order_by = month_num),
         unemp_diff1 = difference(unemp_rate, 
                                 differences = 1, 
                                 order_by = month_num))
...
```{r}
save.image("dyna_regress_prep.Rdata")
#nyc_ts_2 %>% model(ARIMA(total_waste_total ~ avg_precip))
```
...
lin_model_fit <- nyc_ts_2 %>%
  model(
    ...
    liner = TSLM(tw_diff1 ~ cpi_diff1 + unemp_diff1 + avg_precip_diff1 + temp_diff1 + cdd_diff1)
  )
...
```{r}
nyc_ts_2 %>%
  left_join(residuals(lin_model_fit), by = "month_num") %>%
