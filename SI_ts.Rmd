...
library(modeltime)
library(zoo)
load("C:\\Users\\Daniel\\Desktop\\R\\STAT_790\\STAT_790_Case_Seminar\\SI_ts_data.Rdata")
```
...
From the first PACF $\text{1 + ARIMA(0,0,0)}$ we see significant values at lags 3 and 4. Lag 3 has a negative PACF value, indicating that we should begin with an MA(2) or MA(3) model on the undifferenced data. We also see that there is a significant positive value on lag 12. Because we know that our time series data is seasonal, we can look to add a seasonal AR parameter on the undifferenced data. 

## $\text{ARIMA(0,0,2)}$ with constant

Let's begin with $\text{1 + ARIMA(0,0,2)}$
```{r}
si_arima002_fit_cons <- si_ts2 %>% 
  model(arima002_constant = ARIMA(total_waste ~ 1 + 
                                     pdq(0,0,2)))

zoo_arima002_fit <- arima(DSNY_SI_zoo_ts, order = 1 + c(0,0,2))
#names(zoo_arima000_fit)
res_arima002 <- zoo_arima002_fit$residuals
acf(res_arima002, lag.max = 36)
pacf(res_arima002, lag.max = 36)
accuracy(si_arima002_fit_cons)[4]
```

The seasonal lags are still significant on the ACF plot. On the PACF plot, the first three lags are not significant. However, lags (4,8,9) are significant. Along with lag 12, this still indicates that we should add a seasonal AR term on the undifferenced data. We do a lower RMSE value on this model.

## $\text{ARIMA(0,0,2)(1,0,0)}$ with constant and seasonal

$\text{ARIMA(0,0,2)(1,0,0)}_{12}$

```{r}
si_arima002_100_fit_cons <- si_ts2 %>% 
  model(arima002_100_constant = ARIMA(total_waste ~ 1 + 
                                    pdq(0,0,2) +
                                    PDQ(1,0,0, period = 12)))

zoo_arima002_100_fit <- arima(DSNY_SI_zoo_ts, 
                          order = 1 + c(0,0,2),
                          seasonal = list(order = c(1, 0L, 0L),
                                          period = 12))
#names(zoo_arima000_fit)
res_arima002_100 <- zoo_arima002_100_fit$residuals
acf(res_arima002_100, lag.max = 36)
pacf(res_arima002_100, lag.max = 36)
accuracy(si_arima002_100_fit_cons)[4]
```

The bad thing we see here is that the RMSE has increased.The majority of the lags in the ACF plot are contain within the bounds. The first six lags in the PACF plot are contain within significant bounds, however, lags 12 and 36 are not. 
...
AIC = 1800. In the ACF plot, the majority of the lags are not significant. However, the seasonal lags are significant and positive. In the PACF plot, lags 1-5 are not significant, but we see a significant and positive lag at 6, negative at lags (9,10). The seasonal lags spike again, so lets address that first. 

## $\text{ARIMA(2,1,3)(1,0,0)}$ with constant

$\text{1 + \{ARIMA(2,1,3)(1,0,0)_{12}}}$
...
Models with low RMSE: $\text{1 + ARIMA(0,0,2)}$ with $\text{RMSE=1328.737	}$ and 
$\text{1 + \{ARIMA(2,1,3)(1,0,0)_{12}}}$ with $\text{RMSE=1371.112}$
...
```{r}
si_arima002_fit_cons %>% gg_tsresiduals()
```

### Preliminary forecast of si_arima002_fit_cons

```{r}
si_arima002_fit_cons %>% 
  forecast(h = 4) %>% 
  autoplot(si_ts2)
```
